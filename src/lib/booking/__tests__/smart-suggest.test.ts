...
describe("rankAndRecommendSlots", () => {
  ...
  it("recommends earliest slots when no occupied slots exist", () => {
    ...
    // 11:00 and 14:00 should be recommended (highest early-day bonus)
    expect(recommended.map((s) => s.startTime).sort()).toEqual(["11:00", "14:00"]);
  });
  ...
  it("scores slot right after occupied higher than slot right before", () => {
    // Occupied at 15:00.
    // 16:00: after-bonus=10, early-day=max(0,5-(16-11))=0, total=10
    // 14:00: before-bonus=8, early-day=max(0,5-(14-11))=2, total=10
    // Tie → earlier wins. Use slots where after clearly beats before:
    // Occupied at 12:00.
    // 13:00: after-bonus=10, early-day=max(0,5-(13-11))=3, total=13
    // 11:00: before-bonus=8, early-day=max(0,5-(11-11))=5, total=13
    // Still tied. Let's use occupied=17:00:
    // 18:00: after-bonus=10, early-day=max(0,5-7)=0, total=10
    // 16:00: before-bonus=8, early-day=max(0,5-5)=0, total=8
    ...
    expect(recommended[0].startTime).toBe("18:00"); // after-bonus wins
  });
  ...
  it("breaks score ties by earlier start time", () => {
    // No occupied slots. 11:00 gets early bonus 5, 12:00 gets 4, etc.
    // With maxRecommendations=1, 11:00 should win.
    const slots = [makeSlot(15), makeSlot(11), makeSlot(13)];
    const result = rankAndRecommendSlots(slots, new Set(), 1);

    const recommended = result.filter((s) => s.isRecommended);
    expect(recommended[0].startTime).toBe("11:00");
  });
});
